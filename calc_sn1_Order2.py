...
OUT_DAT = "sn1_order.dat"
OUT_PNG = "sn1_order.png"
...
# PA PC PA / PA PC PA / ...
# sn-1 = first PA in each lipid block
sn1_pa_residues = [
    res for i, res in enumerate(u.residues)
    if i % 3 == 0 and res.resname == "PA"
]

print("sn-1 PA residues:", len(sn1_pa_residues))
...
# Build sn-1 C-H pairs from mapping
for key, info in mapping.items():
    m = re.match(r"M_G1C(\d+)_M$", key)
    if not m:
        continue
    ...
    for hindex in [1, 2, 3]:
        hkey = f"M_G1C{cnum_universal}H{hindex}_M"
...
print("Number of sn-1 C-H pairs:", len(pairs))
...
for ts in u.trajectory:
    ...
    for label, cres, cname, hres, hname in pairs:
        ...
        # For acyl chains, use only sn-1 PA residues
        if cres == "PA":
            C_atoms = []
            for res in sn1_pa_residues:
...
if len(labels) == 0:
    raise RuntimeError("No sn-1 order parameters were calculated.")
...
plt.xlabel("sn-1 carbon")
plt.ylabel("Order parameter")
plt.title("Order Parameters - DPPC - sn-1 - AMBER")
plt.legend()
plt.grid(True)
plt.tight_layout()
